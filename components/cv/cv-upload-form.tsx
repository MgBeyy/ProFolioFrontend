...
export default function CvUploadForm() {
  ...
  const {
    ...
    error,
    data,
  } = useMutation((file: File) => cvApi.uploadCV(file), {
    onSuccess: (response) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {data && (
          <Alert className="bg-green-50 border-green-200">
            ...
            <AlertDescription className="text-green-700">
              {data.message || "Özgeçmişiniz başarıyla yüklendi. Analiz sonuçlarınız hazırlanıyor."}
            </AlertDescription>
          </Alert>
        )}
        ...
        {error && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {error.message || "Özgeçmişiniz yüklenirken bir hata oluştu. Lütfen tekrar deneyin."}
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
      <CardFooter className="flex justify-between">
        ...
        <Button className="bg-teal-600 hover:bg-teal-700" disabled={!file || loading} onClick={handleUpload}>
          {loading ? <Loading size="sm" /> : data ? "Yüklendi" : "Özgeçmişi Yükle"}
        </Button>
      </CardFooter>
    </Card>
  )
